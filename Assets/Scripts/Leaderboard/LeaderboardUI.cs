...
namespace Celeris.Leaderboard
{
    ...
    public class LeaderboardUI : MonoBehaviour
    {
        ...
        private IEnumerator LoadLeaderboardRoutine()
        {
            ...
            // Pausa minima para que el spinner sea visible al menos un frame.
            yield return new WaitForSeconds(0.3f);

            // Disparar la peticion. HandleLeaderboardData (suscrito en OnEnable)
            // es el unico responsable de actualizar la UI cuando el evento llega.
            // Esta coroutine solo gestiona el timeout de red — sin polling frame-by-frame.
            SupabaseManager.Instance?.FetchLeaderboard(null, topLimit);

            const float timeoutSeconds = 10f;
            float waited = 0f;
            while (_isLoading && waited < timeoutSeconds)
            {
            ...
            if (_isLoading)
            {
                // HandleLeaderboardData no fue invocado en el tiempo esperado.
        ...
        {
            _isLoading = false;   // Libera el timeout en LoadLeaderboardRoutine.
